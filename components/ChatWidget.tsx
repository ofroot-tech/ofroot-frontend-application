        ...
        <LiquidReveal as="div" active={visible} mode={open ? 'open' : 'close'} options={{ spring: { stiffness: 380, damping: 34 } }} id="ofroot-chat-widget" className="w-80 max-w-[90vw] rounded-xl border border-gray-200 bg-white shadow-2xl" role="dialog" aria-modal="false" aria-label="OfRoot Assistant chat window">
          ...
          <div className="max-h-72 overflow-y-auto px-4 py-3 space-y-3 text-sm">
            {messages.map((message) => (
              <div
                ...
                className={
                  message.from === 'user'
                    ? 'ml-auto max-w-[80%] rounded-lg bg-[#FF9312] px-3 py-2 text-white'
          ...
          <div className="border-t px-3 py-2">
            <textarea
              ...
              className="w-full resize-none rounded-md border border-gray-300 px-3 py-2 text-sm focus:border-[#FF9312] focus:outline-none focus:ring-2 focus:ring-[#FF9312]"
              disabled={sending}
            />
            <div className="mt-2 flex justify-end">
              <button
                ...
                onClick={sendMessage}
                disabled={sending}
                className="inline-flex items-center gap-2 rounded-md bg-[#FF9312] px-3 py-2 text-sm font-semibold text-white hover:bg-[#E07F00] disabled:opacity-60"
              >
