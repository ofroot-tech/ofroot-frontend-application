    ...
    <main className="min-h-screen bg-[#F4F7FA] text-[#071225]">
      <section className="relative overflow-hidden bg-[#071225] px-5 pb-16 pt-32 text-white sm:px-8 sm:pb-24 sm:pt-40">
        ...
        <div className="relative mx-auto max-w-6xl">
          <div className="flex items-center gap-3 font-mono text-xs uppercase tracking-[0.18em] text-[#37FFE0]"><span className="h-2 w-2 rounded-full bg-[#37FFE0]" /> OfRoot / AI readiness audit</div>
          <div className="mt-8 grid gap-10 lg:grid-cols-[1.15fr_1fr] lg:items-end">
            <div>
              ...
              <p className="mt-6 max-w-xl text-lg leading-relaxed text-slate-300">Can it find your business, understand your site, and take the next step? See what stands in the way—and let OfRoot help fix it.</p>
      ...
      ].map(([title, text]) => <div key={title}><h2 className="text-xl font-semibold">{title}</h2><p className="mt-3 text-sm leading-relaxed text-slate-600">{text}</p></div>)}</div><p className="mt-7 max-w-3xl text-sm text-slate-500">Scanning powered by <a href="https://ora.ai" className="underline">Ora</a>. OfRoot turns the findings into practical implementation work. Website mode uses Ora’s essentials methodology; product mode includes its broader integration checks. Scores do not guarantee AI visibility.</p></section>}
    </main>
  );
}
