...
export default function GrowthAuditForm() {
  ...
  if (state === 'success') return <div className="rounded-3xl border border-emerald-200 bg-emerald-50 p-8" role="status"><CheckCircle2 className="mb-4 h-8 w-8 text-emerald-700" /><h2 className="text-2xl font-black text-slate-950">Your audit request is in.</h2><p className="mx-0 mt-3 text-slate-700">We will review the bottleneck and reply by email with the recommended next step and scheduling details when there is a useful fit.</p></div>;
  ...
  return <form onSubmit={submit} onFocus={start} className="rounded-3xl border border-slate-200 bg-white p-6 shadow-[0_20px_60px_rgba(15,23,42,.08)] sm:p-8" aria-describedby={message ? 'form-error' : undefined}>
    <div className="mb-6 border-b border-slate-200 pb-6">
      <p className="mx-0 text-xs font-bold uppercase tracking-[.15em] text-[#B55B00]">Audit request</p>
      <h2 className="mt-2 text-2xl font-black text-slate-950">Tell us what needs to work better.</h2>
      <p className="mx-0 mt-2 text-sm text-slate-600">Four answers are required. Everything under “Add helpful context” is optional.</p>
    </div>
    <div className="grid gap-5 sm:grid-cols-2">
      <label className="text-sm font-semibold">Business name <span className="text-[#9A4A00]">*</span><input className={input} name="businessName" required autoComplete="organization" /></label>
      <label className="text-sm font-semibold">Contact email <span className="text-[#9A4A00]">*</span><input className={input} name="email" type="email" required autoComplete="email" /></label>
      <label className="text-sm font-semibold sm:col-span-2">Main growth bottleneck <span className="text-[#9A4A00]">*</span><select className={input} name="growthBottleneck" required defaultValue=""><option value="" disabled>Select one</option><option>Discoverability</option><option>Lead conversion</option><option>Automation</option><option>Internal knowledge</option><option>Private AI</option><option>Reporting</option><option>Not sure yet</option></select></label>
      <label className="text-sm font-semibold sm:col-span-2">Desired outcome <span className="text-[#9A4A00]">*</span><textarea className={`${input} min-h-28 resize-y`} name="desiredOutcome" required placeholder="What should work better after this engagement?" /></label>
    </div>
    <details className="mt-6 rounded-2xl border border-slate-200 bg-slate-50">
      <summary className="flex min-h-12 cursor-pointer items-center px-4 py-3 text-sm font-bold text-slate-800 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#FF9312] focus-visible:ring-inset">Add helpful context <span className="ml-2 font-normal text-slate-500">(optional)</span></summary>
      <div className="grid gap-5 border-t border-slate-200 p-4 sm:grid-cols-2">
        <label className="text-sm font-semibold sm:col-span-2">Current website<input className={input} name="website" type="url" placeholder="https://" autoComplete="url" /></label>
        ...
        <label className="text-sm font-semibold">Budget range<select className={input} name="budget" defaultValue=""><option value="">Prefer to discuss</option><option>Under $10,000</option><option>$10,000–$25,000</option><option>$25,000–$75,000</option><option>$75,000+</option></select></label>
      </div>
    </details>
    {message && <p id="form-error" className="mx-0 mt-5 text-sm font-semibold text-red-700" role="alert">{message}</p>}
    <button type="submit" disabled={state === 'submitting'} className="mt-6 inline-flex min-h-12 w-full items-center justify-center gap-2 rounded-full bg-[#FF9312] px-6 py-3 font-bold text-slate-950 transition hover:bg-[#ffad42] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#C96800] focus-visible:ring-offset-2 disabled:cursor-wait disabled:opacity-70">{state === 'submitting' && <Loader2 className="h-4 w-4 animate-spin" />}Send audit request</button>
    <p className="mx-auto mt-4 text-center text-xs text-slate-500">No payment is collected. We use this information only to review and respond to your request.</p>
  </form>;
}
