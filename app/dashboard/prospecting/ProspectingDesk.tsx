...
import {CalendarClock, ClipboardCopy, Download, FileUp, Filter, PhoneCall, Sparkles, Upload} from 'lucide-react';
import {buildProspects, callScript, emailDraft, normalizeProspect, parseCsv, prospectKey, scoreProspect, scoreReasons, type OutreachOutcome, type OutreachStatus, type Prospect} from './prospecting-utils';

const STORAGE_KEY = 'ofroot.prospecting-desk.v1';

type QueueBucket = 'today' | 'overdue' | 'upcoming' | 'needs_planning';
type QueueView = QueueBucket | 'all';
...
export function ProspectingDesk() {
  ...
  const queueCounts = useMemo(() => prospects.reduce<Record<QueueBucket, number>>((counts, item) => {
  ...
  const matching = useMemo(() => prospects
    ...
    .filter((item) => `${item.businessName} ${item.website} ${item.email}`.toLowerCase().includes(query.toLowerCase())), [prospects, vertical, market, query]);
  const filtered = useMemo(() => matching
    .filter((item) => queueView === 'all' || actionState(item, today) === queueView)
    .sort((a, b) => compareQueue(a, b, today)), [matching, queueView, today]);
  const selected = filtered.find((item) => item.id === selectedId) || filtered[0] || null;
  const reachable = prospects.filter((item) => item.email || item.phone).length;
  const namedContacts = prospects.filter((item) => item.email && item.contactName).length;
  const sourceHealth = useMemo(() => Array.from(new Map(prospects.map((item) => [item.source, {source: item.source, count: 0, reachable: 0, latest: item.importedAt}])).values()).map((source) => { const items = prospects.filter((item) => item.source === source.source); return {...source, count: items.length, reachable: items.filter((item) => item.email || item.phone).length, latest: items.map((item) => item.importedAt).sort().at(-1) || ''}; }).sort((a, b) => b.latest.localeCompare(a.latest)), [prospects]);
  ...
  return <div className="space-y-5 text-slate-900">
    <section className="overflow-hidden rounded-[24px] border border-slate-200 bg-[#10212b] text-white shadow-[0_18px_45px_rgba(15,35,47,0.16)]">
      <div className="grid gap-5 px-5 py-6 sm:px-7 lg:grid-cols-[1fr_auto] lg:items-end"><div className="max-w-2xl"><div className="flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.17em] text-orange-200"><Sparkles size={14} /> Prospecting desk</div><h1 className="mt-3 text-3xl font-semibold tracking-[-0.035em] text-white sm:text-4xl">Turn evidence into the next useful conversation.</h1><p className="mt-3 max-w-xl text-sm leading-6 text-slate-300">Rank businesses by verified signals, then make each outreach step concrete. Nothing sends from this workspace.</p></div><div className="flex flex-wrap gap-2"><button type="button" onClick={exportProspects} disabled={!prospects.length} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl border border-white/20 px-4 py-2 text-sm font-semibold text-white disabled:cursor-not-allowed disabled:opacity-50"><Download size={17} /> Export</button><ImportButton onFile={onFile} /></div></div>
      <div className="grid border-t border-white/10 bg-white/[0.035] sm:grid-cols-4"><Stat label="Reachable now" value={String(reachable)} /><Stat label="Due today" value={String(queueCounts.today)} /><Stat label="Overdue" value={String(queueCounts.overdue)} /><Stat label="Named email contacts" value={String(namedContacts)} /></div>
    </section>
    <section className="grid gap-3 rounded-2xl border border-[#dbe3e7] bg-[#f7f4ed] p-3 shadow-[0_1px_2px_rgba(15,35,47,0.05)] lg:grid-cols-[1fr_auto] lg:items-center"><div className="flex flex-col gap-2 sm:flex-row"><label className="flex min-h-10 flex-1 items-center gap-2 rounded-xl border border-slate-200 bg-white px-3 text-sm text-slate-500 focus-within:border-[#c56a20] focus-within:ring-2 focus-within:ring-orange-100"><Filter size={15} /><span className="sr-only">Search prospects</span><input className="w-full bg-transparent text-slate-900 outline-none placeholder:text-slate-400" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search business, site, or email" /></label><select aria-label="Action queue view" className="min-h-10 rounded-xl border border-slate-200 bg-white px-3 text-sm" value={queueView} onChange={(event) => setQueueView(event.target.value as QueueView)}><option value="today">Due today ({queueCounts.today})</option><option value="overdue">Overdue ({queueCounts.overdue})</option><option value="upcoming">Upcoming ({queueCounts.upcoming})</option><option value="needs_planning">Needs planning ({queueCounts.needs_planning})</option><option value="all">All prioritized</option></select><select aria-label="Vertical" className="min-h-10 rounded-xl border border-slate-200 bg-white px-3 text-sm" value={vertical} onChange={(event) => setVertical(event.target.value)}>{verticals.map((item) => <option key={item}>{item}</option>)}</select><select aria-label="Market" className="min-h-10 rounded-xl border border-slate-200 bg-white px-3 text-sm" value={market} onChange={(event) => setMarket(event.target.value)}>{markets.map((item) => <option key={item}>{item}</option>)}</select></div><p className="px-1 text-xs text-slate-600">Browser-only data · Scores use vertical, contact detail, and verified evidence only.</p></section>
    {sourceHealth.length ? <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-[0_4px_14px_rgba(15,35,47,0.05)]"><div className="flex items-baseline justify-between"><div><h2 className="text-sm font-semibold">Source health</h2><p className="mt-0.5 text-xs text-slate-500">Latest imports and available contact routes.</p></div><span className="text-xs text-slate-500">{sourceHealth.length} source{sourceHealth.length === 1 ? '' : 's'}</span></div><div className="mt-3 grid gap-2 sm:grid-cols-2 lg:grid-cols-3">{sourceHealth.map((source) => <div key={source.source} className="rounded-xl bg-slate-50 p-3"><div className="truncate text-xs font-semibold text-slate-800" title={source.source}>{source.source}</div><div className="mt-1 text-xs text-slate-600">{source.count} prospects · {source.reachable} reachable</div><div className="mt-1 text-[11px] text-slate-500">Imported {source.latest ? new Date(source.latest).toLocaleDateString() : 'unknown'}</div></div>)}</div></section> : null}
    ...
    {prospects.length === 0 ? <EmptyState onFile={onFile} /> : selected ? <div className="grid gap-5 xl:grid-cols-[minmax(300px,.75fr)_minmax(0,1.25fr)]"><Queue items={filtered} selected={selected} select={setSelectedId} queueView={queueView} today={today} /><Detail prospect={selected} copied={copied} onChange={patchSelected} onCopy={copy} today={today} /></div> : <NoQueueResults queueView={queueView} count={matching.length} onViewChange={setQueueView} />}
  </div>;
}
