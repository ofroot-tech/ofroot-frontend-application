...
## Evidence: Navbar CTA cascade repair and local verification
- Date: 2026-08-10
- Graph node: N14
- Command or verification method: Canonical Playwright measurement at 1440px, 1024px, and 390px; focused `app/globals.css` and `Navbar.tsx` inspection; `git diff --check`; `npx eslint app/components/Navbar.tsx`; `npx tsc --noEmit`; `npm run lint`; `NEXT_PUBLIC_API_BASE_URL=http://127.0.0.1:8002 npm run build`; production-like `next start`; Playwright screenshots and computed-style/interaction checks at 1440px, 1024px, and 390px.
- Result: Canonical production computed the desktop CTA near 20px type with only 2px side padding because broad unlayered anchor rules won the CSS cascade. The scoped component repair now renders a 65px header, 14px desktop navigation type, a 44px audit CTA, 16px / 8px side padding, 10px content gap, a 24px arrow surface, and no freestanding divider. Desktop has no horizontal overflow at 1440px or 1024px. `/book?source=nav`, `/book?source=mobile-nav`, `audit_cta_clicked`, `nav`, and `mobile_nav` remain unchanged. At 390px the mobile dialog opens, locks scrolling, shows its CTA, closes with Escape, restores focus, restores scrolling, and does not overflow. Diff, focused lint, TypeScript, repository lint, and the 168-page Turbopack build exited 0. Repository lint/build repeated only known unrelated warnings. Local runtime also reported the expected unauthenticated `/api/auth/me` 401 and provider-only `/_vercel/insights/script.js` 404; neither originates in this navbar change.
- Exit status: 0
- Remaining uncertainty: The change is local only. No commit, push, preview, merge, provider deployment, or canonical production verification was performed for N14.

## Evidence: Navbar CTA reviewed release and canonical verification
- Date: 2026-08-10
- Graph node: N15
- Command or verification method: Explicit user waiver of the pre-push grill; clean `origin/main` refresh and flight-recorder baseline; scoped commit and push; GitHub PR/check inspection; temporary expiring Vercel preview access; exact preview browser measurements at 1440px, 1024px, and 390px; squash merge through the connected GitHub account; Vercel commit/deployment readback; cache-busted canonical HTTP checks; canonical desktop/mobile browser measurements and interaction checks; Graph Loop consistency checker.
- Result: Commit `ac93c12538d8a131349f5075f22e1f51e61ad600` used the required `ofroot-tech` identity and changed only `Navbar.tsx` plus the existing Graph records. PR #28 passed GitGuardian, Vercel, and Vercel Preview Comments. Exact preview deployment `dpl_3V2ncMT5ySdBcg2qtXFtKFQFN64F` matched the PR head and rendered the intended CTA contract. PR #28 squash-merged as `11345031099a72b8aecad72114416972a025cf74`; `origin/main` and Vercel metadata matched that SHA. Production deployment `dpl_DqScFF8gRjT7L7xw3d8zrrVWUgfB` reached `READY` and attached `www.ofroot.technology` and `ofroot.technology`. Canonical `/` and `/book` returned HTTP 200. At 1440px and 1024px the header measured 65px, the CTA 44px with 14px type, 16px / 8px padding, 10px gap, a 24px arrow surface, no divider, and no overflow. At 390px the 48px mobile CTA remained visible; scroll locking, Escape close, focus restoration, scroll restoration, and no overflow passed. The consistency checker returned completed with no issues or repairs. An unintended empty Vercel project named `navbar-cta-refinement`, created by an unlinked CLI probe, was verified to contain zero deployments and deleted; the real `main-website` project was not altered by that cleanup.
- Exit status: 0
- Remaining uncertainty: Release proof establishes source, provider, and canonical runtime state, not downstream conversion lift. The deleted empty Vercel project cannot be recovered, but it contained no deployments or application data.
